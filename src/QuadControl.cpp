...
float QuadControl::AltitudeControl(float posZCmd, float velZCmd, float posZ, float velZ, Quaternion<float> attitude, float accelZCmd, float dt)
{
  ...
  float desAccel = kpVelZ * (velZCmd - velZ) + KiPosZ * integratedAltitudeError + accelZCmd - float(CONST_GRAVITY);
...
float QuadControl::YawControl(float yawCmd, float yaw)
{
  ...
  /*
  * old code: the code pass the control test, but cause out of control in 10 MagUpdate test.
  * the problem solved, after limit yawError to the [-180, 180] interval.
  */
